...
class UserService {
  ...
  static Future<void> updateLocation({
    ...
    await ApiMethods.authorizedPut('users/me/location', {
      'latitude': latitude,
      'longitude': longitude,
      'location': location,
    });
  ...
  static Future<List<FollowUser>> getNearbyUsers({
    double? latitude,
    double? longitude,
    double? maxDistance,
    int page = 1,
    ...
    final data = await ApiMethods.authorizedGet('users/nearby?$queryParams');
    final users = data['data']?['users'] as List<dynamic>? ?? [];
    return users
        .map((u) => FollowUser.fromApi(Map<String, dynamic>.from(u as Map)))
        .toList();
