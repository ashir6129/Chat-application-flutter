...
export async function listUserConversations(userId, { limit = 30, offset = 0, includePending = false }) {
  const statusFilter = includePending ? '' : "AND cm.status = 'accepted'";
  const result = await query(
    `SELECT c.id, c.type, c.title, c.avatar_url, c.last_message_at, c.created_at, c.metadata,
            cm.last_read_at, cm.status,
            COALESCE(
              (SELECT COUNT(*)::int
               FROM messages m
...
export async function listPendingRequests(userId, { limit = 30, offset = 0 }) {
  const result = await query(
    `SELECT c.id, c.type, c.title, c.avatar_url, c.last_message_at, c.created_at, c.metadata,
            cm.last_read_at, cm.status,
            COALESCE(
...
export async function updateConversationStatus(conversationId, userId, status) {
  const result = await query(
    `UPDATE conversation_members
     SET status = $3
     WHERE conversation_id = $1 AND user_id = $2
     RETURNING conversation_id, user_id, status`,
